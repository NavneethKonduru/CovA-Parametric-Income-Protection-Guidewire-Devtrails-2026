...
const DEMO_USERS = {
  ...
  'qcommerce@cova.in': { password: 'cova2026', role: 'qcommerce', name: 'Demo Q-Commerce', redirectPath: '/qcommerce' }
};
...
/**
 * Express middleware: require Telemetry Auth Secret
 * Used exclusively for high-frequency IoT/App telemetry ingestion endpoints.
 */
function requireTelemetryAuth(req, res, next) {
  const authHeader = req.headers.authorization;
  const expectedSecret = process.env.TELEMETRY_AUTH_SECRET;
  
  if (!expectedSecret) {
    // If not configured, deny access securely or warn in dev
    if (process.env.NODE_ENV === 'development') {
      console.warn('[AUTH] TELEMETRY_AUTH_SECRET is not set. Allowing access in DEV mode only.');
      return next();
    }
    return res.status(500).json({ error: 'Server misconfiguration: Telemetry auth secret missing' });
  }
  ...
  if (token !== expectedSecret) {
    return res.status(403).json({ error: 'Invalid telemetry auth token' });
  }
