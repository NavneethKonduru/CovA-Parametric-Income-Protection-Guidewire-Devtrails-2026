 ...
 */
async function injectGhostWorkers(pgClient, options = {}) {
  const {
    count = 15,
    targetZone = 'ZONE_B',
  ...
  const dataMode = pgClient.getDataMode ? pgClient.getDataMode() : 'demo';
  await pgClient.query("DELETE FROM workers WHERE id LIKE 'GHOST_%' AND data_mode = $1", [dataMode]);
  await pgClient.query("DELETE FROM claims WHERE worker_id LIKE 'GHOST_%' AND data_mode = $1", [dataMode]);
  ...
  const insertQuery = `
    INSERT INTO workers (id, name, phone, zone, platform, archetype, hourly_rate, status, enrolled_date, is_simulated)
  ...
  try {
    ...
    console.error('[FRAUD-INJECTOR] Transaction failed:', err);
    throw err;
  }
 ...
 * @param {object} db - better-sqlite3 instance
 */
async function cleanupGhostWorkers(pgClient) {
  const dataMode = pgClient.getDataMode ? pgClient.getDataMode() : 'demo';
  const deleted = await pgClient.query("DELETE FROM workers WHERE id LIKE 'GHOST_%' AND data_mode = $1", [dataMode]);
  await pgClient.query("DELETE FROM claims WHERE worker_id LIKE 'GHOST_%' AND data_mode = $1", [dataMode]);
  console.log(`[FRAUD-INJECTOR] Cleaned up ${deleted.rowCount} ghost workers`);
  return { cleaned: deleted.rowCount };
}
