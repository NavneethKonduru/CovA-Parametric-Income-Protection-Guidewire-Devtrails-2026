...
const { getTimeSlot } = require('./payout');
...
/** EMA smoothing factor: higher = more reactive (demo mode defaults to faster reactions) */
const EMA_ALPHA = process.env.COVA_MODE === 'demo' ? 0.65 : 0.35;
...
/**
 * Normalize raw civic disruption data into a 0–1 score.
 * @param {object} rawCivic - { is_active }
 * @returns {number} Civic score 0.0–1.0
 */
function normalizeCivicScore(rawCivic) {
  if (!rawCivic || typeof rawCivic !== 'object') return 0;
  return rawCivic.is_active ? 0.95 : 0;
}

/**
 * Normalize telemetry drop data into a 0–1 score.
 * High ratio of stationary workers = high score.
 * @param {object} rawTelemetryDrop - { total_workers, stationary_workers }
 * @returns {number}
 */
function normalizeTelemetryDropScore(rawTelemetryDrop) {
  if (!rawTelemetryDrop || typeof rawTelemetryDrop !== 'object') return 0;
  const total = rawTelemetryDrop.total_workers || 1;
  const stationary = rawTelemetryDrop.stationary_workers || 0;
  if (total <= 0) return 0;
  const ratio = stationary / total;
  return parseFloat(Math.min(ratio * 1.5, 1.0).toFixed(4));
}

/**
 * Normalize historical anomaly into a 0–1 score.
 * Compares current CDI to baseline.
 * @param {object} rawHistorical - { current_cdi, baseline_cdi }
 * @returns {number}
 */
function normalizeHistoricalScore(rawHistorical) {
  if (!rawHistorical || typeof rawHistorical !== 'object') return 0;
  const current = rawHistorical.current_cdi || 0;
  const baseline = rawHistorical.baseline_cdi || 0;
  if (baseline <= 0) return current > 0.5 ? 0.8 : 0;
  const spike = (current - baseline) / baseline;
  if (spike <= 0) return 0;
  return parseFloat(Math.min(spike / 2, 1.0).toFixed(4));
}
 ...
 * CDI = (0.35 × weather) + (0.25 × demand) + (0.15 × civic) + (0.10 × telemetry_drop) + (0.10 × historical) + (0.05 × peer)
 *
 * @param {number} weatherScore
 * @param {number} demandScore
 * @param {number} peerScore
 * @param {number} civicScore
 * @param {number} telemetryDropScore
 * @param {number} historicalScore
 * @param {object} [customWeights]
 * @returns {number} CDI value 0.0–1.0
 */
function calculateCDI(weatherScore, demandScore, peerScore, civicScore = 0, telemetryDropScore = 0, historicalScore = 0, customWeights) {
  const w = customWeights || WEIGHTS;

  let effectiveDemand = demandScore;
  if (DECORRELATE_SIGNALS) {
    // Decorrelate demand from weather and civic
    const maxExternal = Math.max(weatherScore, civicScore);
    effectiveDemand = demandScore * (1 - 0.4 * maxExternal);
  }
  ...
  const cdi = (w.weather * weatherScore) +
              ...
              (w.peer * peerScore) +
              (w.civic * civicScore) +
              (w.telemetry_drop * telemetryDropScore) +
 ...
 *
 * @param {object} scores - { weatherScore, demandScore, peerScore, civicScore, telemetryDropScore, historicalScore }
 ...
 * @param {object} [options.rawCivic] - Raw civic data for narrative
 * @param {object} [options.customWeights] - Optional CDI weight overrides
 * @returns {object} Full disruption analysis result
 */
function analyzeDisruption(scores, options = {}) {
  const { weatherScore, demandScore, peerScore, civicScore = 0, telemetryDropScore = 0, historicalScore = 0 } = scores;
  const { zone, baseThreshold, rawWeather, rawDemand, rawCivic, customWeights } = options;

  // 1. Calculate raw CDI
  const rawCDI = calculateCDI(weatherScore, demandScore, peerScore, civicScore, telemetryDropScore, historicalScore, customWeights);
  ...
  // Apply trigger strategies
  if (CDI_STRATEGY === 'any_dominant') {
    if (weatherScore >= 0.65 || demandScore >= 0.65 || peerScore >= 0.65 || civicScore >= 0.65) {
...
// ──────────────────────────────────────────────
//  LATE ARRIVAL — RETROACTIVE CLAIM BUILDER
// ──────────────────────────────────────────────

/**
 * Build a retroactive claim context from historical disruption data.
 *
 * When a worker was offline during a storm (phone dead, no signal, app
 * in airplane mode) and their app later syncs the buffered telemetry,
 * we need to reconstruct what the CDI *was* at the time their telemetry
 * was recorded — not what it is now.
 *
 * @param {object} params
 * @param {object} params.worker         - Worker row from DB
 * @param {string} params.zone           - Zone the telemetry places them in
 * @param {string} params.telemetryStart - ISO timestamp of earliest buffered fix
 * @param {string} params.telemetryEnd   - ISO timestamp of latest buffered fix
 * @param {object} params.historicalDisruption - Row from disruption_events near the telemetry window
 * @param {object} [params.telemetryPayload] - Raw telemetry for fraud engine
 * @returns {object} Retroactive claim context ready for /api/claims/trigger
 */
function buildRetroactiveClaimContext({
  worker,
  zone,
  telemetryStart,
  telemetryEnd,
  ...
  // Reconstruct signal scores from the historical disruption event
  const weatherScore = parseFloat(historicalDisruption.weather_score) || 0;
  const demandScore  = parseFloat(historicalDisruption.demand_score)  || 0;
  const peerScore    = parseFloat(historicalDisruption.peer_score)    || 0;
  const historicalCDI = parseFloat(historicalDisruption.cdi)          || 0;

  // Determine disruption type from the historical condition
  const condition = (historicalDisruption.condition || '').toLowerCase();
  ...
  else if (condition.includes('outage'))                disruptionType = 'PLATFORM_OUTAGE';
  else if (condition.includes('curfew') || condition.includes('suspended')) disruptionType = 'CIVIC_CURFEW';

  // Calculate hours lost from the telemetry window (capped at 8)
  const startMs = new Date(telemetryStart).getTime();
  const endMs   = new Date(telemetryEnd).getTime();
  const rawHoursLost = Math.max(0, (endMs - startMs) / (1000 * 60 * 60));
  const hoursLost = Math.min(parseFloat(rawHoursLost.toFixed(2)), 8);

  // Time slot based on when the disruption *actually* happened
  const disruptionHour = new Date(telemetryStart).getHours();
  const timeSlot = getTimeSlot(disruptionHour);

  return {
    workerId: worker.id,
    zone,
    disruptionType,
    hoursLost: hoursLost || 4, // fallback to 4h if telemetry window is too short
    weatherScore,
    demandScore,
    peerScore,
    disruptionStartedAt: telemetryStart,
    isRetroactive: true,
    retroactiveContext: {
      originalDisruptionTimestamp: historicalDisruption.timestamp,
      historicalCDI,
      triggerLevel: historicalDisruption.trigger_level || 'standard',
      syncedAt: new Date().toISOString(),
      telemetryWindow: { start: telemetryStart, end: telemetryEnd },
      timeSlot,
    },
    telemetry: telemetryPayload || {
      workerId: worker.id,
      lat: parseFloat(worker.lat) || 12.9716,
      lng: parseFloat(worker.lng) || 77.5946,
      gnss_variance: 5.0,
      velocity: 2.0,
      zone_entry: telemetryStart,
      cn0Array: [28, 32, 35, 31, 29, 33],
      gpsHistory: [
        { lat: parseFloat(worker.lat) || 12.9716, lon: parseFloat(worker.lng) || 77.5946, timestamp: startMs },
        { lat: parseFloat(worker.lat) || 12.9716, lon: parseFloat(worker.lng) || 77.5946, timestamp: endMs },
      ],
    },
  };
}
