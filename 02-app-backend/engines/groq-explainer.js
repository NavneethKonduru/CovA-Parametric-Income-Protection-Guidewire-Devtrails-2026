...
try {
  if (process.env.GROQ_API_KEY) {
    groqClient = new Groq({ 
      ...
      maxRetries: 1, // Enforce 1 retry maximum to avoid hanging batches
      timeout: 3000 // 3000ms hard timeout
    });
...
// Track metrics
const metrics = { successes: 0, fallbacks: 0 };
...
const RATE_LIMIT_COOLDOWN_MS = 60000; // 1 minute pause after 429
...
async function generateExplanation(claim, worker, fraudResult) {
  ...
  if (now - lastRateLimitTime < RATE_LIMIT_COOLDOWN_MS) {
    return generateTemplateExplanation(status, context) + " [AI: Fallback/Rate-Limited]";
  }
  ...
  if (groqClient) {
    ...
    while (retries >= 0) {
      try {
        console.log(`[GROQ] Calling API for claim ${claim.id} (retries left: ${retries})...`);
        ...
        if (explanation) {
          metrics.successes++;
          return explanation + " [AI: Groq/llama-3.3]";
        }
      } catch (e) {
        if (e.status === 429) {
          console.warn(`[GROQ] Rate limit hit for ${claim.id}. Backing off ${delay}ms...`);
          lastRateLimitTime = Date.now();
          await sleep(delay);
          delay *= 3; // Aggressive exponential backoff
          retries--;
        } else {
          console.warn(`[GROQ] API call failed for ${claim.id}: ${e.message}. Using template.`);
          break;
        }
      }
    }
  ...
  metrics.fallbacks++;
  if (metrics.fallbacks % 10 === 0) {
    console.warn(`[GROQ_METRICS] Fallbacks reached ${metrics.fallbacks}. AI generation is currently degraded.`);
  }
...
async function generateBatchExplanations(claims, workers, fraudResults) {
  const results = [];
  const CONCURRENCY_LIMIT = 2; // Lowered to avoid rate limits
  ...
  for (let i = 0; i < claims.length; i += CONCURRENCY_LIMIT) {
    ...
    if (i + CONCURRENCY_LIMIT < claims.length) await sleep(250); // Small pause
  }
