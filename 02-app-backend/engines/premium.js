...
// Base rate cached in memory — updated async when pg.js is available
let _cachedBaseRate = 35;

/**
 * Refresh cached config from PostgreSQL (call periodically or on startup).
 * Non-blocking — premium calculation always has a fallback.
 */
async function refreshPremiumConfig() {
  try {
    const pg = require('../data/pg');
    const val = await pg.getInsurerConfig('base_premium_rate');
    if (val) _cachedBaseRate = val;
  } catch (e) {
    // pg not available yet — use cached value
  }
}
// Refresh every 60 seconds
setInterval(refreshPremiumConfig, 60000);
setTimeout(refreshPremiumConfig, 3000); // Initial load after 3s
...
function calculatePremium(zone, archetype, options = {}) {
  const baseRate = _cachedBaseRate;
