...
export default function CDIGauge({ signals = {}, cdi = 0, threshold = 0.6 }) {
  ...
  const dominantFactor = useMemo(() => {
    if (weatherVal === 0 && demandVal === 0 && peerVal === 0) return 'None';
    if (weatherVal >= demandVal && weatherVal >= peerVal) return 'Weather-driven';
    if (demandVal >= weatherVal && demandVal >= peerVal) return 'Demand-driven';
    return 'Peer-driven';
  }, [weatherVal, demandVal, peerVal]);
    ...
    <div style={{ padding: '0' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.4)', marginLeft: '0.2rem' }}>• {dominantFactor}</span>
        </div>
        ...
        <div style={{ marginTop: '1.5rem', fontSize: '0.65rem', color: 'rgba(255,255,255,0.3)', textAlign: 'center' }}>
